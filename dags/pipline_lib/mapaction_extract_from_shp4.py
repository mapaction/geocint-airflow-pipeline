...
def clip_shapefile_by_country(country_geojson_path, input_shp_path, output_name):
    ...
    logging.info("Reading input shapefile (points)...")
    input_gdf = gpd.read_file(input_shp_path)

    if input_gdf.crs != country_gdf.crs:
        logging.info(f"Reprojecting points to match CRS of country boundary: {country_gdf.crs}")
    ...
    logging.info(f"Saving the resulting points shapefile to {output_shp_path}...")
